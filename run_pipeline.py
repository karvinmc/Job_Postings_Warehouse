...
# Keep pipeline order explicit because later steps depend on earlier outputs.
PIPELINE_STEPS: list[Step] = [
    ...
    Step(
        "opportunity_mart",
        "05_create_opportunity_mart.sql",
        "opportunity_mart.snapshot_job_opportunity",
    ),
    Step(
        "update_opportunity_mart",
        "06_update_opportunity_mart.sql",
        [
            "staging_job_opportunities",
            "opportunity_mart.snapshot_job_opportunity",
        ],
    ),
]

# One row is written per step/table combination so individual outputs can be monitored.
...
def run_step(con: duckdb.DuckDBPyConnection, run_id: str, step: Step) -> bool:
    ...
    # Normalize the config so the rest of the function can always iterate over a list.
    ...
    try:
        ...
        if not tables_to_count:
            # DDL-only steps have no output table to validate.
            print(
                f"[{step.name}] SUCCESS — no table configured to count "
        ...
        for table in tables_to_count:
            ...
            try:
                ...
                # A count failure should not hide the fact that the SQL itself succeeded.
                rows_summary.append(f"{table}=ERROR")
                print(
                    f"[{step.name}] could not count rows in "
                    f"'{table}': {count_err}"
                )
        ...
        print(
            f"[{step.name}] SUCCESS — {', '.join(rows_summary)} "
            f"duration={duration:.2f}s"
        )
        return True
...
def run_pipeline(db_path: str, only_step: str | None) -> int:
    ...
    print(
        ...
        f"{'SUCCESS' if all_ok else 'FAILED'} ==="
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the job-postings-warehouse ETL pipeline."
    )
    ...
    parser.add_argument(
        "--step",
        default=None,
        help="Run only this step (see --list for valid names). "
